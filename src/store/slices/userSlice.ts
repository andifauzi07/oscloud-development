...
export const fetchCurrentUser = createAsyncThunk(
	...
	async (_, { rejectWithValue }) => {
		try {
			...
			const { data, error } = await supabase.from('app_user').select('*').eq('userid', session.data.session.user.id).single();
...
export const fetchUsers = createAsyncThunk(
	...
	async ({ workspaceId, filters }: { workspaceId: number; filters?: UserFilters }, { rejectWithValue }) => {
		try {
			let query = supabase.from('app_user').select('*').eq('workspaceid', workspaceId);
...
export const fetchUserById = createAsyncThunk(
	...
	async ({ workspaceId, userId }: { workspaceId: number; userId: string }, { rejectWithValue, getState }) => {
		try {
			...
			const { data, error } = await supabase.from('app_user').select('*').eq('workspaceid', workspaceId).eq('userid', userId).single();
...
export const updateUser = createAsyncThunk('user/update', async ({ workspaceId, userId, data }: { workspaceId: number; userId: string; data: UpdateUserData }, { rejectWithValue }) => {
	try {
		const { data: updatedUser, error } = await supabase.from('app_user').update(data).eq('workspaceid', workspaceId).eq('userid', userId).single();
...
export const deleteUser = createAsyncThunk('user/delete', async ({ workspaceId, userId }: { workspaceId: number; userId: string }, { rejectWithValue }) => {
	try {
		const { error } = await supabase.from('app_user').delete().eq('workspaceid', workspaceId).eq('userid', userId);
