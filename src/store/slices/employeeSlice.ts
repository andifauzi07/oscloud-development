...
// export const fetchWorkspaceEmployees = createAsyncThunk('employee/fetchByWorkspace', async ({ workspaceId, filters }: { workspaceId: number; filters?: EmployeeFilters }) => {
...
// 		});
// 	}
// 	const response = await apiClient.get(`/workspaces/${workspaceId}/employees${queryString ? `?${queryString}` : ''}`);
...
export const fetchWorkspaceEmployees = createAsyncThunk('employee/fetchByWorkspace', async ({ workspaceId, filters }: { workspaceId: number; filters?: EmployeeFilters }) => {
	...
	const queryString = params.toString(); // <-- Dipindahkan ke sini agar `params` sudah berisi data
	const response = await apiClient.get(`/workspaces/${workspaceId}/employees${queryString ? `?${queryString}` : ''}`);
	return response.data;
});

export const fetchEmployeeById = createAsyncThunk('employee/fetchOne', async ({ workspaceId, employeeId }: { workspaceId: number; employeeId: number }) => {
...
export const updateEmployee = createAsyncThunk('employee/update', async ({ workspaceId, employeeId, data }: { workspaceId: number; employeeId: number; data: Partial<Employee> }) => {
	const response = await apiClient.put(`/workspaces/${workspaceId}/employees/${employeeId}`, data);
	return response.data;
});

export const deleteEmployee = createAsyncThunk('employee/delete', async ({ workspaceId, employeeId }: { workspaceId: number; employeeId: number }) => {
	await apiClient.delete(`/workspaces/${workspaceId}/employees/${employeeId}`);
	return employeeId;
});

export const fetchEmployeeCategories = createAsyncThunk('employee/fetchCategories', async (workspaceId: number) => {
...
export const createEmployeeCategory = createAsyncThunk(
	...
	async ({
		...
		const response = await apiClient.post(`/workspaces/${workspaceId}/employees/employee-categories`, data);
...
export const updateEmployeeCategory = createAsyncThunk(
	...
	async ({
		...
		const response = await apiClient.put(`/workspaces/${workspaceId}/employees/employee-categories/${categoryId}`, data);
		return response.data.category;
	}
);

export const deleteEmployeeCategory = createAsyncThunk('employee/deleteCategory', async ({ workspaceId, categoryId }: { workspaceId: number; categoryId: number }) => {
	await apiClient.delete(`/workspaces/${workspaceId}/employees/employee-categories/${categoryId}`);
	return categoryId;
});
	...
	extraReducers: (builder) => {
		builder
			...
			.addCase(updateEmployee.fulfilled, (state, action) => {
				...
				// Pastikan data baru valid
				if (!updatedEmployee || !updatedEmployee.employeeid) return;

				// Cari data karyawan yang sesuai dan perbarui
				...
				if (index !== -1) {
					state.employees[index] = {
						...state.employees[index],
						...updatedEmployee,
					}; // Memastikan data lama tidak tertimpa jika ada data yang tidak dikirim oleh API
				} else {
					state.employees.push(updatedEmployee); // Jika data tidak ditemukan, tambahkan data baru
				}

				if (state.selectedEmployee?.employeeid === updatedEmployee.employeeid) {
