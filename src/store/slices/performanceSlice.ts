...
export const fetchSheetById = createAsyncThunk(
    ...
    async ({
        ...
        try {
            ...
            // Extract the specific sheet from the array
            const sheet = response.data.sheets.find(
                (s: any) => s.sheetId === filters.sheetId
            );
            if (!sheet) {
        ...
        builder
            ...
            .addCase(fetchSheetById.fulfilled, (state, action) => {
                ...
                const index = state.sheets.findIndex(
                    (s) => s.sheetId === action.payload.sheetId
                );
                if (index !== -1) {
...
export const { clearCurrentSheet } = performanceSlice.actions;
