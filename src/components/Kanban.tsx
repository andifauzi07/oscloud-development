...
export const KanbanBoard = ({ children, addBoard }: any) => {
    return (
        <div className="flex flex-col gap-4 p-4 overflow-x-auto md:flex-row">
            {children}
            <Button onClick={addBoard} className="flex-shrink-0 w-12 h-12">
                <Plus size={16} />
            </Button>
        </div>
    );
...
export const KanbanCard = ({
    ...
    return (
        <div
            ...
            {...attributes}
            {...listeners}
            className="p-4 mb-2 bg-white border border-gray-200 shadow-sm"
        >
                ...
                <KanbanBoard addBoard={addBoard}>
                    {boards.map((board) => (
                        <KanbanColumn
                            ...
                            addItem={addItem}
                            totalContractValue={board.items.reduce(
                                (sum, item) => sum + parseFloat(item.contractValue.replace("$", "").replace(",", "")),
                                0
