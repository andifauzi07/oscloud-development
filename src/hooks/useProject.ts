...
export const useProjects = (filters?: {
  ...
  const memoizedFilters = useMemo(
    () => filters,
    [filters?.keyword, filters?.startDate, filters?.endDate, filters?.status, filters?.page, filters?.limit]
  );
...
export const useProject = () => {
  ...
  const { currentUser } = useUserData();
  const workspaceid = currentUser?.workspaceid;
  const { projects, currentProject, projectLeads, loading, error } = useSelector((state: RootState) => state.project);
  ...
  const updateCategory = useCallback(
    async (categoryId: number, data: UpdateCategoryRequest) => {
      if (!workspaceid) throw new Error("Workspace ID is required");
      return dispatch(
        updateProjectCategory({
          workspaceId: Number(workspaceid),
