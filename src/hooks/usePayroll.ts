...
export const usePayrollEmployees = () => {
  ...
  const { employees, loading, error } = useSelector((state: RootState) => state.payroll);
...
export const usePayments = (filters?: Record<string, any>) => {
  ...
  const { currentUser } = useUserData();
  const workspaceid = currentUser?.workspaceid;
  const { payments, loading, error } = useSelector((state: RootState) => state.payroll);
...
export const useCreatePayment = () => {
  ...
  const handleCreatePayment = async (data: any) => {
    ...
    return dispatch(createPayment({ workspaceId: Number(workspaceid), data })).unwrap();
  };
...
export const useEmployeeProfile = (employeeId: number) => {
  ...
  const { currentUser } = useUserData();
  const workspaceid = currentUser?.workspaceid;
  const { employeeProfile, loading, error } = useSelector((state: RootState) => state.payroll);
...
export const useEmployeeProjects = (employeeId: number) => {
  ...
  const { currentUser } = useUserData();
  const workspaceid = currentUser?.workspaceid;
  const { employeeProjects, loading, error } = useSelector((state: RootState) => state.payroll);
...
export const useEmployeePayments = (employeeId: number) => {
  ...
  const { currentUser } = useUserData();
  const workspaceid = currentUser?.workspaceid;
  const { employeePayments, loading, error } = useSelector((state: RootState) => state.payroll);
