...
function RouteComponent() {
	...
	const { companies, loading, addCompany, total, currentPage, perPage } = useCompanies(filters, debouncedSearchKeyword, page, limit);
	...
	useEffect(() => {
		localStorage.setItem('companyColumnSettings', JSON.stringify(columnSettings));
	}, [columnSettings]);

	// Function to generate columns from settings (NOW INSIDE THE COMPONENT)
	const generateColumns = useCallback((settings: ColumnSetting[]): ColumnDef<Company>[] => {
		return settings
			...
			.map((setting) => {
				...
				switch (setting.accessorKey) {
					...
					case 'personnel':
						...
						break;
					case 'category_group':
						columnDef.cell = ({ row }) => <span className="px-2 py-1 text-xs whitespace-nowrap">{row.original.category_group}</span>;
						break;
					case 'city':
						columnDef.cell = ({ row }) => <div className="flex flex-wrap gap-1 text-xs whitespace-nowrap">{row.original.city}</div>;
						break;
		...
		<div className="flex flex-col flex-1 h-full">
			...
			<div className="flex justify-end flex-none bg-white">
				<AddRecordDialog
					...
					nonEditableColumns={['logo*', 'companyId*', 'detail*', 'personnel*']}></AddRecordDialog>
				{editButton()}
			</div>
				...
				<div className="max-w-full overflow-x-auto">
					<div className="">
						<DataTable
							...
							onSave={handleSave}
							nonEditableColumns={['logo*', 'companyId*', 'detail*', 'personnel*']}
						/>
					</div>
				</div>
			</div>
