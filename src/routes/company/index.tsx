...
function RouteComponent() {
    ...
    const { companies, loading, addCompany, updateCompany, total } =
    ...
    useEffect(() => {
        console.log('Settings after status filter:', 
    ...
    const filteredCompanies = useMemo(() => {
        return transformedCompanies.filter((company) => {
            ...
            if (statusFilter && statusFilter !== "All") {
                const hasMatchingStatus = company.personnel.some(
                    (p: { status: string }) => p.status === statusFilter
                );
            ...
            if (debouncedSearchKeyword) {
                ...
                return (
                    ...
                    (company.product?.toLowerCase() || "").includes(
                        searchLower
                    ) ||
                    (company.category_group?.toLowerCase() || "").includes(
                        searchLower
                    )
                );
            }
        ...
        return (
            <>
                ...
                {isEditable ?
                    <Button
                        onClick={async () => {
                            const result = await handleSaveEdits(
                                filteredCompanies,
                                updateDataFromChild,
                                "companyid",
                                [
                    ...
                    <DataTable
                        ...
                        setTableData={(updateFunctionOrData) => {
                            if (typeof updateFunctionOrData === "function") {
                                const newData =
                                    updateFunctionOrData(updateDataFromChild);
