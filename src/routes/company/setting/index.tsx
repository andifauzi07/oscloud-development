...
function RouteComponent() {
	...
	const generateSettingColumns = useMemo(() => {
		return (setColumnSettings: React.Dispatch<React.SetStateAction<ColumnSetting[]>>): ColumnDef<ColumnSetting>[] => {
			return [
				{
					accessorKey: 'label',
					header: () => <h1 className="pl-8">Data field shown in the Company</h1>,
					cell: ({ row }) => <h1 className="pl-8 py-2">{row.original.label}</h1>,
				},
				{
					accessorKey: 'type',
					header: 'Type',
	...
	const handleDrop = useCallback(
		(e: React.DragEvent<HTMLDivElement>, dropKey: string) => {
			e.preventDefault();
	...
	return (
		<div className="flex flex-col flex-1 w-full h-full">
			...
			<div className="px-8 py-4 bg-white border-r"></div>
