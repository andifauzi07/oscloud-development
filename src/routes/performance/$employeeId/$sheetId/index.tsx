...
function RouteComponent() {
	...
	useEffect(() => {
		if (sheet?.scores) {
			const initialScores = sheet.scores.reduce(
				(acc, score) => ({
					...acc,
	...
	return (
		<div className="flex flex-col h-full">
			{/* Header Sections */}
			<div className="flex-none min-h-0 px-2 py-2 bg-white border-b border-r">
			...
			<div className="flex-none min-h-0 px-2 py-2 bg-gray-100 border-b border-r">
			...
			<div className="flex-none min-h-0 px-8 py-2 border-b border-r">
				<h1 className="text-base">{sheet.template.name}</h1>
			</div>
			...
			<div className="flex justify-end flex-none bg-white">
				<Button
					...
					className="w-20 h-10 text-black bg-transparent border-l border-r cursor-pointer">
					{isEditing ? 'SAVE' : 'EDIT'}
				</Button>
			</div>
			...
			<div className="grid h-full grid-cols-2">
				{/* Score Chart Section */}
				<Card className="flex flex-col h-full border-l-0 rounded-none">
					...
					{template.categories.map((category) => (
						<div
							...
							<div
								...
								<div className="flex-1 p-4 border-l-0">
									<span className="font-medium text-white">{category.categoryname}</span>
								</div>
								<div className="flex border-t">
									<div className="w-20 p-4 text-center text-white border-r">Score</div>
									<div className="w-20 p-4 text-center text-black bg-white border-r">Weight</div>
								</div>
							</div>
							...
							{category.points.map((point) => (
								<div
									...
									<div className="flex items-center">
										...
										<div className="w-20 p-4 text-center border-t border-l border-r">{point.weight}</div>
									</div>
								</div>
							))}
						</div>
