...
const columnsEmployee: ColumnDef<any>[] = [
    ...
    // },
    {
        ...
        cell: ({ row }) =>
            (
                row.original.rates?.find((rate: any) => rate.type === "A")
                    ?.ratevalue
            ) ?
                `${row.original.rates.find((rate: any) => rate.type === "A").ratevalue.toLocaleString()}`
            :   `${(0.0).toLocaleString()}`,
    },
    {
        id: "hourlyRateB",
        ...
        cell: ({ row }) =>
            (
                row.original.rates?.find((rate: any) => rate.type === "B")
                    ?.ratevalue
            ) ?
                `${row.original.rates.find((rate: any) => rate.type === "B").ratevalue.toLocaleString()}`
            :   `${(0.0).toLocaleString()}`,
    },
    //removed because it is not in the response
    // {
...
function RouteComponent() {
    ...
    const [newProfile, setNewProfile] = useState<EmployeeProfile | null>(
        profile
    );
    ...
    const handleProfileSave = async () => {
        try {
            ...
            if (editedProfile.department)
                updateData.department = editedProfile.department;
            if (editedProfile.category)
                updateData.category = editedProfile.category;
    ...
    const projectColumns = [
        ...
        },
        {
            ...
            header: "Hourly Rate",
            cell: ({ row }: any) =>
                `¥${row.original.hourlyRate.toLocaleString()}`,
        },
        {
            accessorKey: "totalFee",
            header: "Total Fee",
            cell: ({ row }: any) =>
                `¥${row.original.totalFee.toLocaleString()}`,
        },
    ];
        ...
        },
        {
            ...
            cell: ({ row }) =>
                editable ?
                    <Select
                        value={
                            selectValues[row.original.paymentId] ||
                            row.original.status
                        }
                        onValueChange={(value) =>
    ...
    return (
        <div className="flex flex-col flex-1 h-full">
            <Tabs defaultValue="profile">
                            ...
                            <div className="w-[70%] overflow-y-auto">
                                <InfoSection
                                    items={basicInfo.map((item) => ({
                                        ...
                                        value:
                                            editedProfile[
                                                item.key as keyof typeof editedProfile
                                            ]?.toString() || item.value,
                                    }))}
                                ...
                                />
                                <InfoSection items={rateInfo} title="Contact" />
                            </div>
                        </div>
                    </div>
                </TabsContent>
                ...
                {/* Projects Tab */}
                <TabsContent className="m-0" value="joinedprojects">
                ...
                {/* Payments Tab */}
                <TabsContent className="m-0" value="payments">
